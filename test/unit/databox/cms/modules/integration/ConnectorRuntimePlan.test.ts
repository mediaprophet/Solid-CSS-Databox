...
import { planConnectorRuntimeJob } from '../../../../../../src/databox/cms/modules/integration/ConnectorRuntimePlan';
...
const manifest: PortableConnectorManifest = {
  ...
  mapping: {
    ...
    turtle: r2rml,
    rootIri: 'https://pods.example/.databox/cms/mappings/staff.ttl#TriplesMap',
  },
  target: {
    podBaseIri: 'https://pods.example/',
    resourceContainer: 'https://pods.example/.databox/cms/directory/',
    graphIri: 'https://pods.example/.databox/cms/graphs/staff',
  },
};
...
describe('ConnectorRuntimePlan', (): void => {
  it('plans a one-time import as a sidecar command with secret references, not inline credentials.', (): void => {
    ...
    expect(plan).toMatchObject({
      ...
      targetContainer: 'https://pods.example/.databox/cms/directory/',
      targetGraph: 'https://pods.example/.databox/cms/graphs/staff',
      conflict: {
        policy: 'append-only',
  ...
  it('plans live sync as one-way source-to-pod with cursor and provenance placeholders.', (): void => {
    const plan = planConnectorRuntimeJob(manifest, {
      ...
      cursorResource: 'https://pods.example/.databox/cms/connectors/staff/cursor.ttl',
    }, runtime);
    ...
    expect(plan.provenance).toMatchObject({
      ...
      mappingRootIri: 'https://pods.example/.databox/cms/mappings/staff.ttl#TriplesMap',
      jobId: 'staff-sync',
    });

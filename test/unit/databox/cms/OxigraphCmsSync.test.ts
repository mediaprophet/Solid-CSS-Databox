...
import { OxigraphCmsSync } from '../../../../src/databox/cms/OxigraphCmsSync';
import type {
...
const receiptState = 'https://fresh.example/.databox/cms/modules/receipt';
const menuState = 'https://fresh.example/.databox/cms/modules/menu';
...
describe('OxigraphCmsSync', (): void => {
  ...
  it('refuses to process more allowed resources than the configured bound.', async(): Promise<void> => {
    ...
    await expect(sync.syncAll()).rejects.toThrow('CMS Oxigraph sync refused to hydrate 2 resources');

...
} from '../../../../src/databox/ipms/VerticalProfile';
...
describe('IPMS vertical profiles', (): void => {
  it('round-trips a declarative vertical profile bundle as portable Turtle.', async(): Promise<void> => {
    const turtle = await serializeVerticalProfileToTurtle(FOOD_RESTAURANT_VERTICAL_PROFILE, {
      subjectIri: 'https://databox.example/.well-known/databox-ipms/vertical-profiles/food.restaurant.ttl#profile',
    });

    expect(turtle).toContain('ipms:VerticalProfile');
    expect(turtle).toContain('ipms:moduleList');
  ...
  it('applies enabled defaults and RDF config through the module config store.', async(): Promise<void> => {
    ...
    expect(data.get('https://databox.example/.databox/ipms/modules/catalogue'))
      .toContain('https://schema.org/itemListOrder');
    expect(data.get('https://databox.example/.databox/ipms/modules/receipt'))
  ...
  it('ships meaningful lighthouse bundles from the IPMS plan.', (): void => {
    ...
    expect((): void => {
      parseVerticalProfileRdf(`
        ...
        <https://databox.example/profiles/food>
          ...
          ipms:useCaseList ("FOOD").
      `);
    }).toThrow('IPMS vertical profile modules list is required.');
  });
});

...
} from '../../../../../../src/databox/ipms/modules/website/PublicFeedRenderer';
import {
...
describe('renderPublicWebsiteFeed', (): void => {
  it('advertises a IPMS module manifest for the portable public feed renderer.', (): void => {
  ...
  void => {
    ...
    expect(rendered.controlPlanePath).toBe('/.databox/ipms');
    ...
    expect(rendered.jsonLd['@context']).toEqual({ '@vocab': 'https://schema.org/', odrl: 'http://www.w3.org/ns/odrl/2/' });
    ...
    expect(rendered.html).not.toContain('ipmsControlToken');
    expect(rendered.html).not.toContain('/.databox/ipms');
  ...
  it('rejects a protected control-plane path.', (): void => {
    expect((): PublicWebsiteFeedRender => renderPublicWebsiteFeed({
      ...baseInput,
      publicPath: '/.databox/ipms/public-feed',
    })).toThrow('must not be under the protected IPMS control plane');
  });
...
describe('renderPublicWebsiteFeedFromRdf', (): void => {
  it('renders the same public feed from portable schema.org Turtle state.', (): void => {
    ...
    expect(rendered.requiresControlToken).toBe(false);
    expect(JSON.stringify(rendered.jsonLd)).not.toContain('urn:solid-server:databox:ipms#');
  });

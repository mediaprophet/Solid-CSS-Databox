...
`,
  ...
  it('round-trips connector manifests and jobs as ordinary Turtle descriptors.', async(): Promise<void> => {
    const manifestTurtle = await serializeConnectorManifestToTurtle(manifest, {
      subjectIri: 'https://pods.example/.well-known/databox-ipms/connectors/staff.ttl#connector',
    });
    const jobTurtle = await serializeConnectorJobToTurtle(job, {
      subjectIri: 'https://pods.example/.well-known/databox-ipms/connector-jobs/staff.ttl#job',
    });

    expect(manifestTurtle).toContain('ipms:ConnectorManifest');

...
const runner = join(root, 'scripts', 'run-ipms-oxigraph-smoke.mjs');

describe('IPMS Oxigraph smoke runner', (): void => {
  ...
  it('skips gracefully when optional mode has no configured endpoint or launcher.', (): void => {
    ...
    expect(output).toContain('Skipping live IPMS Oxigraph smoke');
  ...
  it('skips gracefully when the optional local Oxigraph binary is unavailable.', (): void => {
    const output = execFileSync('node', [
      ...
      '--start-oxigraph',
      '--skip-when-unavailable',
      '--oxigraph-command=ipms-oxigraph-binary-that-does-not-exist',
    ], {
      cwd: root,
      encoding: 'utf8',
      env: endpointFreeEnv(),

...
const runner = join(root, 'scripts', 'run-ipms-migration-proof.mjs');

describe('IPMS migration proof runner', (): void => {
  it('builds a unified live migration plan with real CSS profile launch commands.', (): void => {
    ...
    expect(plan).toMatchObject({
      ...
      sourceCss: {
        ...
        worksRoute: 'http://127.0.0.1:4910/.databox/ipms/works',
      },
      targetCss: {
        ...
        profile: 'config/ipms/ipms-sparql.json',
        worksImportRoute: 'http://127.0.0.1:4911/.databox/ipms/works/import',
        typeIndexRoute: 'http://127.0.0.1:4911/.well-known/databox-ipms',
      },
      oxigraph: {
    ...
    expect(plan.targetCss.args).toEqual([
      'bin/server.js',
      '-c',
      'config/ipms/ipms-sparql.json',
      '--baseUrl',
      'http://127.0.0.1:4911/',
      '-p',
      '4911',
      '--rootFilePath',
      '.data/ipms-migration/target',
      '--ipmsControlToken',
      '<redacted>',
      '--sparqlEndpoint',
  ...
  it('builds a split endpoint plan for an existing Oxigraph-backed CSS target.', (): void => {
    ...
    expect(plan).toMatchObject({
      ...
      targetCss: {
        ...
        profile: 'config/ipms/ipms-oxigraph.json',
      },
      oxigraphEndpoints: {
    ...
    expect(plan.targetCss.args).toEqual([
      'bin/server.js',
      '-c',
      'config/ipms/ipms-oxigraph.json',
      '--baseUrl',
      'http://localhost:5011/',
      '-p',
      '5011',
  ...
  it('skips gracefully when optional mode has no live CSS or Oxigraph path.', (): void => {
    ...
    expect(output).toContain('Skipping live IPMS migration proof');
    expect(output).toContain('No file-backed IPMS source endpoint');
  });

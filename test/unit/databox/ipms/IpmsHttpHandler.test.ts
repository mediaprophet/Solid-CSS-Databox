...
import { IpmsHttpHandler } from '../../../../src/databox/ipms/IpmsHttpHandler';
...
import { CustomerDisplayStore } from '../../../../src/databox/ipms/CustomerDisplayStore';
...
import { PublicWebsiteStore } from '../../../../src/databox/ipms/PublicWebsiteStore';
import { TableSessionStore } from '../../../../src/databox/ipms/TableSessionStore';
import type { ResourceStore } from '../../../../src/storage/ResourceStore';
import { readableToString } from '../../../../src/util/StreamUtil';

const token = 'ipms-control-token-0123456789012345';
const sameLengthWrong = 'ipms-control-token-9999999999999999';
...
describe('A IpmsHttpHandler', (): void => {
  let registry: InMemoryDataboxModuleRegistry;
  let handler: IpmsHttpHandler;
  ...
  beforeEach((): void => {
    ...
    handler = new IpmsHttpHandler(registry, token);
  });

  function posStoresHandler(): IpmsHttpHandler {
    return new IpmsHttpHandler(
      registry,
      token,
      '/.databox/ipms',
      configStore,
  ...
  it('rejects a control token shorter than 32 bytes.', (): void => {
    expect((): IpmsHttpHandler => new IpmsHttpHandler(registry, 'too-short'))
      .toThrow('at least 32 bytes');
  });

  describe('canHandle()', (): void => {
    it('accepts the exact base and sub-paths.', async(): Promise<void> => {
      await expect(handler.canHandle(input(new MockResponse(), { url: '/.databox/ipms' }))).resolves.toBeUndefined();
      await expect(handler.canHandle(input(new MockResponse(), { url: '/.databox/ipms/modules' })))
        .resolves.toBeUndefined();
    });

    it('accepts the public .well-known manifest discovery resources.', async(): Promise<void> => {
      await expect(handler.canHandle(input(new MockResponse(), { url: '/.well-known/databox-ipms' })))
        .resolves.toBeUndefined();
      await expect(handler.canHandle(input(new MockResponse(), {
        url: '/.well-known/databox-ipms/modules/hosting.ttl',
      }))).resolves.toBeUndefined();
    });
    ...
    it('rejects other paths, including a missing URL.', async(): Promise<void> => {
      await expect(handler.canHandle(input(new MockResponse(), { url: '/other' })))
        .rejects.toThrow('Not a Databox IPMS route.');
      await expect(handler.canHandle(input(new MockResponse(), {})))
        .rejects.toThrow('Not a Databox IPMS route.');
    });

    it('normalises a custom route base without a leading and with a trailing slash.', async(): Promise<void> => {
      const custom = new IpmsHttpHandler(registry, token, 'databox/ipms/');
      await expect(custom.canHandle(input(new MockResponse(), { url: '/databox/ipms/x' })))
        .resolves.toBeUndefined();
  ...
  describe('handle()', (): void => {
    it('publishes a public .well-known manifest index without module state.', async(): Promise<void> => {
      handler = new IpmsHttpHandler(registry, token, '/.databox/ipms', configStore);
      await configStore.setEnabled('hosting', false);

      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.well-known/databox-ipms',
        method: 'GET',
      ...
      expect(parseModuleManifestIndexRdf(res.body)).toMatchObject({
        indexIri: 'http://databox.example/.well-known/databox-ipms',
        manifestUrls: expect.arrayContaining([
          'http://databox.example/.well-known/databox-ipms/modules/hosting.ttl',
          'http://databox.example/.well-known/databox-ipms/modules/receipt.ttl',
        ]),
      });
    });
    ...
    Promise<void> => {
      ...
      expect(res.statusCode).toBe(200);
      expect(res.headers['content-type']).toContain('text/turtle');
      expect(res.body).not.toContain('urn:solid-server:databox:ipms#enabled');
      expect(parseModuleManifestRdf(res.body, {
        subjectIri: 'http://databox.example/.well-known/databox-ipms/modules/hosting.ttl',
      })).toMatchObject({
        id: 'hosting',
        name: 'Hosting',
    ...
    it('returns 404 for a missing public module manifest resource.', async(): Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.well-known/databox-ipms/modules/not-installed.ttl',
        method: 'GET',
      }));
    ...
    it('fails closed instead of publishing malformed registered manifests.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.well-known/databox-ipms/modules/bad.ttl',
        method: 'GET',
      }));

      expect(res.statusCode).toBe(500);
      expect(JSON.parse(res.body)).toEqual({
        error: 'IPMS module manifest name must be a non-empty string.',
      });
    });
    ...
    it('returns 401 without, or with an invalid, bearer token.', async(): Promise<void> => {
      ...
      for (const auth of invalidAuths) {
        const res = new MockResponse();
        await handler.handle(input(res, { url: '/.databox/ipms/modules', method: 'GET', auth }));
    ...
    it('lists module manifests for the built-in route with a valid token.', async(): Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, { url: '/.databox/ipms/modules', auth: `Bearer ${token}` }));
      expect(res.statusCode).toBe(200);
      expect(JSON.parse(res.body)).toEqual(expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          routes: [ 'POST /.databox/ipms/website/preview', 'POST /.databox/ipms/website/publish' ],
        }),
      ]));
    });
    ...
    it('lists lighthouse vertical profiles with missing horizontal module detail.', async(): Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, { url: '/.databox/ipms/vertical-profiles', auth: `Bearer ${token}` }));
    ...
    it('previews vertical profile defaults without mutating module state.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/vertical-profiles/food.restaurant/preview',
        method: 'POST',
        auth: `Bearer ${token}`,
      }));
    ...
    it('applies a vertical profile only when every referenced module is installed.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/vertical-profiles/food.restaurant/apply',
        method: 'POST',
        auth: `Bearer ${token}`,
      }));
    ...
    it('reads and writes module state through the portable RDF config store.', async(): Promise<void> => {
      ...
      await handler.handle(input(write, {
        url: '/.databox/ipms/modules/hosting',
        method: 'PUT',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
          enabled: false,
      ...
      expect(storeData.get('http://localhost:3000/.databox/ipms/modules/hosting'))
        .toContain('urn:solid-server:databox:ipms#enabled');
    ...
    it('exports a portable IPMS works bundle with module manifests and RDF state.', async(): Promise<void> => {
      handler = new IpmsHttpHandler(registry, token, '/.databox/ipms', configStore);
      await configStore.setEnabled('hosting', true);

      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/works',
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
      expect(res.statusCode).toBe(200);
    ...
    it('imports a portable IPMS works bundle through the control plane.', async(): Promise<void> => {
      handler = new IpmsHttpHandler(registry, token, '/.databox/ipms', configStore);
      const bundle = {
        '@context': {},
        type: 'DataboxIpmsWorks',
    ...
    it('builds a printable receipt document from authorized JSON input.', async(): Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/receipt/build',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
    ...
    Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/website/preview',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
          state: {
      ...
      expect(parsed.html).not.toContain('/.databox/ipms');
    });
    ...
    it('persists a waiter POS order flow through the ResourceStore and reads it back.', async(): Promise<void> => {
      handler = new IpmsHttpHandler(registry, token, '/.databox/ipms', configStore, undefined, orderStore);

      const write = new MockResponse();
      await handler.handle(input(write, {
        url: '/.databox/ipms/pos/orders',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: posOrderBody(),
      }));
      ...
      const read = new MockResponse();
      await handler.handle(input(read, {
        url: '/.databox/ipms/pos/orders?iri=http%3A%2F%2Flocalhost%3A3000%2Fpos%2Forders%2Fo-1',
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
      expect(read.statusCode).toBe(200);
    ...
    it('returns 404 when reading a POS order resource that does not exist.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/orders?iri=http%3A%2F%2Flocalhost%3A3000%2Fpos%2Forders%2Fabsent',
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
      expect(res.statusCode).toBe(404);
    ...
    it('opens and persists a cash register session, then reads it back.', async(): Promise<void> => {
      ...
      await handler.handle(input(open, {
        url: '/.databox/ipms/pos/register/sessions',
        method: 'POST',
        auth: `Bearer ${token}`,
      ...
      const read = new MockResponse();
      await handler.handle(input(read, {
        url: '/.databox/ipms/pos/register/sessions?iri=http%3A%2F%2Flocalhost%3A3000%2Fpos%2Fregisters%2Freg-1%2F' +
          'sessions%2Fs-1',
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
    ...
    it('closes a cash register session with a counted total.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/register/sessions/close',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
          session,
    ...
    it('renders and persists a customer display playlist, then reads it back.', async(): Promise<void> => {
      ...
      const read = new MockResponse();
      await handler.handle(input(read, {
        url: '/.databox/ipms/pos/display?iri=http%3A%2F%2Flocalhost%3A3000%2Fpos%2Fdisplay',
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
      expect(read.statusCode).toBe(200);
    ...
    it('publishes rendered public website assets through the ResourceStore.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/website/publish',
        method: 'POST',
        auth: `Bearer ${token}`,
    ...
    it('publishes a sitemap derived from business parameters through the ResourceStore.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/website/sitemap',
        method: 'POST',
        auth: `Bearer ${token}`,
    ...
    it('returns 404 for an unknown authorized route (including a missing URL).', async(): Promise<void> => {
      const res = new MockResponse();
      await handler.handle(input(res, { url: '/.databox/ipms/nope', method: 'GET', auth: `Bearer ${token}` }));
      expect(res.statusCode).toBe(404);
    ...
    it('creates a table session through the ResourceStore.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/tables/sessions',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
          sessionId: 'ts-1',
    ...
    it('closes a table session through the ResourceStore.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/tables/sessions/close',
        method: 'POST',
        auth: `Bearer ${token}`,
    ...
    it('reads a table session back via GET.', async(): Promise<void> => {
      ...
      await handler.handle(input(readRes, {
        url: `/.databox/ipms/pos/tables/sessions?iri=${encodeURIComponent(iri)}`,
        method: 'GET',
        auth: `Bearer ${token}`,
      }));
    ...
    it('pushes incremental display state updates.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/display/state',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({
          displayIri: 'http://localhost:3000/pos/display',
    ...
    it('creates a standalone Wi-Fi onboarding resource.', async(): Promise<void> => {
      ...
      const res = new MockResponse();
      await handler.handle(input(res, {
        url: '/.databox/ipms/pos/wifi-onboarding',
        method: 'POST',
        auth: `Bearer ${token}`,
        body: JSON.stringify({

...
const port = getPort('DataboxIpmsA11y');
const baseUrl = `http://localhost:${port}/`;
const controlToken = 'ipms-a11y-control-token-000000001';

describe('Databox IPMS Accessibility (A11y) & i18n Checks', (): void => {
  let app: App;
  ...
  beforeAll(async(): Promise<void> => {
    ...
    app = await instantiateFromConfig(
      ...
      'config/ipms/ipms.json',
      {
        ...
        'urn:solid-server:ipms:variable:controlToken': 'ipms-a11y-control-token-000000001',
      },
    ) as App;
    await app.start();
  });
  ...
  it('generates HTML with standard a11y markers on the website endpoints', async(): Promise<void> => {
    const res = await fetch(`${baseUrl}.databox/ipms/website/preview`, {
      method: 'POST',
      headers: {

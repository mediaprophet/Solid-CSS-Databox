...
type EndpointConfig = {
  profile: 'cms-sparql.json' | 'cms-oxigraph.json';
...
const port = getPort('DataboxCms') + 100;
const baseUrl = `http://localhost:${port}/`;
const controlToken = 'cms-oxigraph-control-token-00000001';
...
describe('the Databox CMS Oxigraph smoke endpoint configuration', (): void => {
...
describeIfEndpoint('the Databox CMS control plane over a live Oxigraph/SPARQL backend', (): void => {
  let app: App;
  ...
  beforeAll(async(): Promise<void> => {
    ...
    const instances = await instantiateFromConfig(
      ...
      getPresetConfigPath(`cms/${endpointConfig!.profile}`),
      variables,
    ) as App;
  ...
  it('writes CMS module RDF state and round-trips portable works through SPARQL storage.', async(): Promise<void> => {
    ...
    const stateResponse = await fetch(`${modulesRoute}/hosting`, {
      ...
      body: JSON.stringify({
        enabled: false,
        configTurtle: '<> <urn:example:cmsOxigraphSmoke> "written-through-sparql" .',
      }),
    });
    ...
    const importResponse = await fetch(`${baseUrl}.databox/cms/works/import`, {
      method: 'POST',
      headers: {
        authorization: `Bearer ${controlToken}`,
        'content-type': 'application/json',
        ...
        modules: [
          ...works.modules,
          {
            manifest: {
              ...
              version: '0.1.0',
              description: 'Imported by the live SPARQL CMS smoke test.',
              capabilities: [ 'cms:portable-core' ],
              routes: [],
            },
            enabled: true,
            state: {
              contentType: 'text/turtle',
              turtle: '<> <urn:example:cmsOxigraphImportSmoke> "imported-through-sparql" .',
            },
          },
        ],
      }),

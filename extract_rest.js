...
// HostingApi
const hostingRoutes = lines.slice(164, 174).map(l => l.replace(/this\.router\./g, 'router.')).join('\n');
...
// ReceiptApi
const receiptRoutes = lines.slice(174, 185).map(l => l.replace(/this\.router\./g, 'router.')).join('\n');
...
// MenuApi
const menuRoutes = lines.slice(185, 196).map(l => l.replace(/this\.router\./g, 'router.')).join('\n');

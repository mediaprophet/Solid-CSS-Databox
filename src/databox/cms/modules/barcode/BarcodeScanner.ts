...
export const GS1_AIS: Record<string, string> = {
  ...
  8017: 'Global Service Relation Number to identify the relationship between an ' +
    'organisation offering services and the provider of services',
  8018: 'Global Service Relation Number to identify the relationship between an ' +
...
function detectSymbology(raw: string): BarcodeSymbology {
  ...
  if (raw.startsWith('(') && /\(\d+\)/u.test(raw)) {
    return 'GS1-128';
  }
  ...
  if (/^[\d $%+./A-Z-]+$/u.test(raw) && raw.length <= 43) {
    return 'CODE-39';
  }
  return 'UNKNOWN';
}
...
export function parseGs1Data(raw: string): Gs1ParseResult[] {
  ...
  const parenRegex = /\((\d{2,4})\)([^()]+)/gu;
  let match = parenRegex.exec(raw);
  let found = false;
...
export function validateGtinCheckDigit(gtin: string): boolean {
  if (!/^\d{8}$|^\d{12}$|^\d{13}$|^\d{14}$/u.test(gtin)) {
    return false;
  }
  const digits: number[] = [];
  for (const char of gtin) {
...
export function lookupProductByGtin(
  gtin: string,
  catalogue: readonly { productId: string; gtin?: string; name: string }[],
): ProductLookupResult {
  const trimmed = requireNonEmpty(gtin, 'gtin');
  const match = catalogue.find((p): boolean => p.gtin === trimmed);

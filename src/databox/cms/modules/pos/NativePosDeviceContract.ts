...
const TERMS = {
  posDeviceDescriptor: namedNode(`${CMS.namespace}NativePosDeviceDescriptor`),
  posDeviceJob: namedNode(`${CMS.namespace}NativePosDeviceJob`),
  identifier: namedNode(`${SCHEMA}identifier`),
  deviceKind: namedNode(`${CMS.namespace}deviceKind`),
  deviceWebId: namedNode(`${CMS.namespace}deviceWebId`),
  endpoint: namedNode(`${CMS.namespace}endpoint`),
  endpointUrl: namedNode(`${CMS.namespace}endpointUrl`),
  transport: namedNode(`${CMS.namespace}transport`),
  tlsMode: namedNode(`${CMS.namespace}tlsMode`),
  mtlsDeviceWebId: namedNode(`${CMS.namespace}mtlsDeviceWebId`),
  privateNetworkAccess: namedNode(`${CMS.namespace}privateNetworkAccess`),
  capability: namedNode(`${CMS.namespace}capability`),
  roleConstraint: namedNode(`${CMS.namespace}roleConstraint`),
  allowedRole: namedNode(`${CMS.namespace}allowedRole`),
  allowedAgent: namedNode(`${CMS.namespace}allowedAgent`),
  sessionMode: namedNode(`${CMS.namespace}sessionMode`),
  requireActiveSession: namedNode(`${CMS.namespace}requireActiveSession`),
  maxSessionAgeSeconds: namedNode(`${CMS.namespace}maxSessionAgeSeconds`),
  deviceId: namedNode(`${CMS.namespace}deviceId`),
  command: namedNode(`${CMS.namespace}command`),
  status: namedNode(`${CMS.namespace}status`),
  createdAt: namedNode(`${CMS.namespace}createdAt`),
  requestedBy: namedNode(`${CMS.namespace}requestedBy`),
  operatorSession: namedNode(`${CMS.namespace}operatorSession`),
  sessionId: namedNode(`${CMS.namespace}sessionId`),
  sessionWebId: namedNode(`${CMS.namespace}sessionWebId`),
  roleIri: namedNode(`${CMS.namespace}roleIri`),
  startedAt: namedNode(`${CMS.namespace}startedAt`),
  expiresAt: namedNode(`${CMS.namespace}expiresAt`),
  executionTier: namedNode(`${CMS.namespace}executionTier`),
  noBrowserHardwareIo: namedNode(`${CMS.namespace}noBrowserHardwareIo`),
  parameter: namedNode(`${CMS.namespace}parameter`),
  reason: namedNode(`${CMS.namespace}reason`),
  registerId: namedNode(`${CMS.namespace}registerId`),
  pulseMs: namedNode(`${CMS.namespace}pulseMs`),
};
...
export const NATIVE_POS_DEVICE_MODULE_MANIFEST: SolidModuleManifest = {
  ...
  configShape: `${CMS.namespace}NativePosDeviceConfigShape`,
  adminUi: {
...
export function parseNativePosDeviceDescriptorRdf(
  ...
  if (!hasQuad(quads, subject, RDF.terms.type, TERMS.posDeviceDescriptor)) {
    throw new Error(
      `Native POS device descriptor ${subject.value} must declare rdf:type cms:NativePosDeviceDescriptor.`,
    );
  }
...
export function parseNativePosDeviceJobRdf(
  ...
  if (!hasQuad(quads, subject, RDF.terms.type, TERMS.posDeviceJob)) {
    throw new Error(`Native POS device job ${subject.value} must declare rdf:type cms:NativePosDeviceJob.`);
  }
...
function defaultDeviceSubject(id: string): string {
  return `urn:solid-server:databox:cms:pos-device:${encodeURIComponent(id)}`;
}

function defaultJobSubject(id: string): string {
  return `urn:solid-server:databox:cms:pos-job:${encodeURIComponent(id)}`;
}

...
function assertFeedInput(body: unknown): asserts body is FeedInput {
  if (!isRecord(body) || !Array.isArray((body as Record<string, unknown>).products)) {
    throw new TypeError('A feeds request needs a products array.');
  }
  for (const product of (body as Record<string, unknown>).products as Record<string, unknown>[]) {
    if (!isRecord(product) || typeof product.id !== 'string' || typeof product.name !== 'string' || typeof product.price !== 'number' || typeof product.currency !== 'string') {

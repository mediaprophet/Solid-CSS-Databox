 ...
 * the same display without making Oxigraph or the CMS control plane canonical.
 */
...
function validatePublicPath(value: string, label: string): string {
  ...
  if (checked === CONTROL_PLANE_PATH || checked.startsWith(`${CONTROL_PLANE_PATH}/`)) {
    throw new BadRequestHttpError(`A ${label} must not be under the protected CMS control plane.`);
  }
  return checked;
}

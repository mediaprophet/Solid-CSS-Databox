...
import { CMS, DC, LDP, RDF } from '../../util/Vocabularies';
...
export const DEFAULT_CMS_DISCOVERY_PATH = '/.well-known/databox-cms';

const MODULE_PATH_SEGMENT = 'modules';

/**
 * Options shared by the public CMS module-manifest discovery helpers.
 */
export interface ModuleManifestDiscoveryOptions {
  ...
  /** The public discovery index path. Defaults to `/.well-known/databox-cms`. */
...
/**
 * Serialize installed CMS module manifests as a minimal standard-Solid discovery resource.
 *
...
export async function serializeModuleManifestIndexToTurtle(
  ...
  const quads: Quad[] = [
    ...
    rdfQuad(index, DC.terms.title, literal('Databox CMS module manifests')),
    rdfQuad(
      index,
      DC.terms.description,
      literal('Installed CMS module manifests published as ordinary Solid RDF resources.'),
    ),
  ];
...
export function parseModuleManifestIndexRdf(turtle: string, baseIri?: string): ParsedModuleManifestIndex {
  const quads = parseTurtle(turtle, baseIri, 'CMS module manifest discovery index');
  const index = findIndexSubject(quads);
  const manifestUrls = objects(quads, index, LDP.terms.contains).map((term): string => {
    if (term.termType !== 'NamedNode') {
      throw new Error('CMS module manifest discovery index members must be IRIs.');
    }
    return term.value;
  });
  return {
...
export function parseDiscoveredModuleManifests(
  ...
  return index.manifestUrls.map((url): SolidModuleManifest => {
    ...
    if (turtle === undefined) {
      throw new Error(`CMS module manifest resource ${url} is missing.`);
    }
...
function validateModuleId(value: string): string {
  if (!/^[\w.:-]+$/u.test(value)) {
    throw new Error('CMS module manifest id must be a safe module id.');
  }
  return value;
}
...
function findIndexSubject(quads: readonly Quad[]): NamedNode {
  ...
  if (unique.length === 0) {
    throw new Error('CMS module manifest discovery index must declare an LDP container subject.');
  }
  if (unique.length > 1) {
    throw new Error('CMS module manifest discovery index must contain exactly one LDP container subject.');
  }
  if (unique[0].termType !== 'NamedNode') {
    throw new Error('CMS module manifest discovery index subject must be a named node.');
  }
  return unique[0];
}

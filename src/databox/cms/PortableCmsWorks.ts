...
export interface PortableCmsModuleState {
...
export interface PortableCmsWorksBundle {
  readonly '@context': Record<string, string>;
  readonly type: 'DataboxCmsWorks';
  readonly generatedAt: string;
  readonly portability: {
    readonly canonicalStore: 'Solid LDP/RDF resources';
    readonly cssEnhanced: 'CMS control plane is an optional interpreter, not the canonical store';
    readonly backendTargets: readonly string[];
    readonly nonPortableRuntimeWork: readonly string[];
  };
  readonly modules: readonly PortableCmsModuleState[];
...
export interface StandardSolidCmsWorksOptions {
  /** Root Solid storage URL where the CMS works resources are published. */
...
export interface StandardSolidCmsWorksResource {
...
export interface StandardSolidCmsWorksResources {
  readonly typeIndex: StandardSolidCmsWorksResource;
  readonly manifests: readonly StandardSolidCmsWorksResource[];
  readonly verticalProfiles: readonly StandardSolidCmsWorksResource[];
  readonly connectorManifests: readonly StandardSolidCmsWorksResource[];
  readonly connectorJobs: readonly StandardSolidCmsWorksResource[];
  readonly states: readonly StandardSolidCmsWorksResource[];
}

/**
 * Exports the declarative CMS "works": installed module manifests plus pod-backed RDF state.
 *
 ...
 */
export async function exportPortableCmsWorks(
...
): Promise<PortableCmsWorksBundle> {
  const modules = await Promise.all(registry.list().map(async(manifest): Promise<PortableCmsModuleState> => {
...
/**
 * Imports a portable CMS works bundle into a registry and optional Solid/RDF config store.
 *
 ...
 */
export async function importPortableCmsWorks(
  bundle: unknown,
  registry: DataboxModuleRegistry,
  configStore?: ModuleConfigStore,
): Promise<PortableCmsWorksBundle> {
  const parsed = parsePortableCmsWorks(bundle);
 ...
 * for `cms:Module`, and module state is stored as Turtle at the same deterministic paths used by
 * `ModuleConfigStore`. No CSS control-plane endpoint or SPARQL server is required to read these resources.
 */
export async function createStandardSolidCmsWorksResources(
  bundle: unknown,
  options: StandardSolidCmsWorksOptions,
): Promise<StandardSolidCmsWorksResources> {
  const parsed = parsePortableCmsWorks(bundle);
  const paths = standardSolidPaths(options);
  const manifests = await Promise.all(parsed.modules.map(async(module): Promise<StandardSolidCmsWorksResource> => {
  ...
  Promise<StandardSolidCmsWorksResource> => {
  ...
  const states = await Promise.all(parsed.modules.map(async(module): Promise<StandardSolidCmsWorksResource> => {
 ...
 */
export async function publishPortableCmsWorksToStandardSolidStore(
  bundle: unknown,
  store: ResourceStore,
  options: StandardSolidCmsWorksOptions,
): Promise<StandardSolidCmsWorksResources> {
  const resources = await createStandardSolidCmsWorksResources(bundle, options);
 ...
 * CSS-enhanced CMS control plane.
 */
export async function importPortableCmsWorksFromStandardSolidStore(
  store: ResourceStore,
  registry: DataboxModuleRegistry,
  configStore: ModuleConfigStore,
  options: StandardSolidCmsWorksOptions,
  ...
  const verticalProfilePaths = parseTypeIndexResourcePaths(
    typeIndexTurtle,
    paths.typeIndexPath,
    namedNode(`${CMS.namespace}VerticalProfile`),
    'vertical profile',
    false,
  );
  const connectorManifestPaths = parseTypeIndexResourcePaths(
    typeIndexTurtle,
    paths.typeIndexPath,
    namedNode(`${CMS.namespace}ConnectorManifest`),
    'connector manifest',
    false,
  );
  const connectorJobPaths = parseTypeIndexResourcePaths(
    typeIndexTurtle,
    paths.typeIndexPath,
    namedNode(`${CMS.namespace}ConnectorJob`),
    'connector job',
    false,
  );
  const modules = await Promise.all(manifestPaths.map(async(path): Promise<PortableCmsModuleState> => {
  ...
  return importPortableCmsWorks({
    ...
    portability: portableCmsWorksPortability(),
    modules,
    verticalProfiles,
    connectorManifests,
    connectorJobs,
...
function parsePortableCmsWorks(bundle: unknown): PortableCmsWorksBundle {
  const record = requireRecord(bundle, 'CMS works bundle');
  if (record.type !== 'DataboxCmsWorks') {
    throw new Error('CMS works bundle type must be DataboxCmsWorks.');
  }
  const modules = requireArray(record.modules, 'CMS works modules')
    .map((module, index): PortableCmsModuleState => parseModuleState(module, index));
  const verticalProfiles = record.verticalProfiles === undefined ?
      [] :
      requireArray(record.verticalProfiles, 'CMS works verticalProfiles')
  ...
  const connectorManifests = record.connectorManifests === undefined ?
      [] :
      requireArray(record.connectorManifests, 'CMS works connectorManifests')
  ...
  const connectorJobs = record.connectorJobs === undefined ?
      [] :
      requireArray(record.connectorJobs, 'CMS works connectorJobs')
        .map((job, index): PortableConnectorJob => parseConnectorJob(job, index));
  const connectors = validateConnectorWorks(connectorManifests, connectorJobs);
  return {
    '@context': requireRecord(record['@context'], 'CMS works @context') as Record<string, string>,
    type: 'DataboxCmsWorks',
    generatedAt: requireString(record.generatedAt, 'CMS works generatedAt'),
...
function portableCmsWorksPortability(): PortableCmsWorksBundle['portability'] {
  return {
    canonicalStore: 'Solid LDP/RDF resources',
    cssEnhanced: 'CMS control plane is an optional interpreter, not the canonical store',
    backendTargets: [
...
function parsePortability(value: unknown): PortableCmsWorksBundle['portability'] {
  const portability = requireRecord(value, 'CMS works portability');
  return {
    canonicalStore: requireExact(
      portability.canonicalStore,
      'Solid LDP/RDF resources',
      'CMS works portability canonicalStore',
    ),
    cssEnhanced: requireExact(
      portability.cssEnhanced,
      'CMS control plane is an optional interpreter, not the canonical store',
      'CMS works portability cssEnhanced',
    ),
    backendTargets: requireStringArray(portability.backendTargets, 'CMS works portability backendTargets'),
    nonPortableRuntimeWork: requireStringArray(
      portability.nonPortableRuntimeWork,
      'CMS works portability nonPortableRuntimeWork',
    ),
  };
}

function parseModuleState(value: unknown, index: number): PortableCmsModuleState {
  const module = requireRecord(value, `CMS works module ${index}`);
  const state = module.state === undefined ? undefined : parseState(module.state, index);
  return {
    manifest: parseManifest(module.manifest, index),
    enabled: requireBoolean(module.enabled, `CMS works module ${index} enabled`),
    ...state === undefined ? {} : { state },
  };
}

function parseManifest(value: unknown, index: number): SolidModuleManifest {
  const manifest = requireRecord(value, `CMS works module ${index} manifest`);
  const adminUi = manifest.adminUi === undefined ? undefined : parseAdminUi(manifest.adminUi, index);
  return {
    id: requireModuleId(manifest.id, `CMS works module ${index} manifest id`),
    name: requireString(manifest.name, `CMS works module ${index} manifest name`),
    version: requireString(manifest.version, `CMS works module ${index} manifest version`),
    description: requireString(manifest.description, `CMS works module ${index} manifest description`),
    capabilities: requireStringArray(manifest.capabilities, `CMS works module ${index} manifest capabilities`),
    routes: requireStringArray(manifest.routes, `CMS works module ${index} manifest routes`),
    ...manifest.configShape === undefined ?
        {} :
        {
          configShape: requireString(manifest.configShape, `CMS works module ${index} manifest configShape`),
        },
...
function parseAdminUi(value: unknown, index: number): SolidModuleManifest['adminUi'] {
  const adminUi = requireRecord(value, `CMS works module ${index} manifest adminUi`);
  return {
    navLabel: requireString(adminUi.navLabel, `CMS works module ${index} manifest adminUi navLabel`),
    path: requireString(adminUi.path, `CMS works module ${index} manifest adminUi path`),
  };
}

function parseState(value: unknown, index: number): PortableCmsModuleState['state'] {
  const state = requireRecord(value, `CMS works module ${index} state`);
  return {
    contentType: requireExact(state.contentType, 'text/turtle', `CMS works module ${index} state contentType`),
    turtle: requireString(state.turtle, `CMS works module ${index} state turtle`),
  };
}

function parseVerticalProfile(value: unknown, index: number): VerticalProfileManifest {
  const profile = requireRecord(value, `CMS works vertical profile ${index}`);
  return validateVerticalProfileBundle({
    id: requireModuleId(profile.id, `CMS works vertical profile ${index} id`),
    name: requireString(profile.name, `CMS works vertical profile ${index} name`),
    version: requireString(profile.version, `CMS works vertical profile ${index} version`),
    description: requireString(profile.description, `CMS works vertical profile ${index} description`),
    useCases: requireStringArray(profile.useCases, `CMS works vertical profile ${index} useCases`),
    modules: requireArray(profile.modules, `CMS works vertical profile ${index} modules`)
...
function parseVerticalProfileModule(
  ...
  const module = requireRecord(value, `CMS works vertical profile ${profileIndex} module ${moduleIndex}`);
  ...
  return {
    moduleId: requireModuleId(
      module.moduleId,
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} moduleId`,
    ),
    required: requireBoolean(
      module.required,
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} required`,
    ),
    enabledByDefault: requireBoolean(
      module.enabledByDefault,
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} enabledByDefault`,
    ),
    rationale: requireString(
      module.rationale,
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} rationale`,
    ),
...
function parseVerticalProfileDefaultConfig(
  ...
  const config = requireRecord(
    value,
    `CMS works vertical profile ${profileIndex} module ${moduleIndex} defaultConfig`,
  );
  return {
    contentType: requireExact(
      config.contentType,
      'text/turtle',
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} defaultConfig contentType`,
    ),
    turtle: requireString(
      config.turtle,
      `CMS works vertical profile ${profileIndex} module ${moduleIndex} defaultConfig turtle`,
    ),
  };
}

function parseConnectorManifest(value: unknown, index: number): PortableConnectorManifest {
  const connector = requireRecord(value, `CMS works connector manifest ${index}`);
  const source = requireRecord(connector.source, `CMS works connector manifest ${index} source`);
  const mapping = requireRecord(connector.mapping, `CMS works connector manifest ${index} mapping`);
  const target = requireRecord(connector.target, `CMS works connector manifest ${index} target`);
  return validatePortableConnectorManifest({
    id: requireModuleId(connector.id, `CMS works connector manifest ${index} id`),
    name: requireString(connector.name, `CMS works connector manifest ${index} name`),
    version: requireString(connector.version, `CMS works connector manifest ${index} version`),
    description: requireString(connector.description, `CMS works connector manifest ${index} description`),
    source: {
      kind: requireString(source.kind, `CMS works connector manifest ${index} source kind`) as
        PortableConnectorManifest['source']['kind'],
      sourceRef: requireString(source.sourceRef, `CMS works connector manifest ${index} source sourceRef`),
      ...source.label === undefined ?
          {} :
          { label: requireString(source.label, `CMS works connector manifest ${index} source label`) },
    },
    modes: requireStringArray(
      connector.modes,
      `CMS works connector manifest ${index} modes`,
    ) as PortableConnectorManifest['modes'],
    mapping: {
      language: requireString(
        mapping.language,
        `CMS works connector manifest ${index} mapping language`,
      ) as PortableConnectorManifest['mapping']['language'],
      contentType: requireExact(
        mapping.contentType,
        'text/turtle',
        `CMS works connector manifest ${index} mapping contentType`,
      ),
      turtle: requireString(mapping.turtle, `CMS works connector manifest ${index} mapping turtle`),
      ...mapping.rootIri === undefined ?
          {} :
          { rootIri: requireString(mapping.rootIri, `CMS works connector manifest ${index} mapping rootIri`) },
    },
    target: {
      podBaseIri: requireString(target.podBaseIri, `CMS works connector manifest ${index} target podBaseIri`),
      ...target.resourceContainer === undefined ?
          {} :
          {
            resourceContainer: requireString(
              target.resourceContainer,
              `CMS works connector manifest ${index} target resourceContainer`,
            ),
          },
      ...target.graphIri === undefined ?
          {} :
          { graphIri: requireString(target.graphIri, `CMS works connector manifest ${index} target graphIri`) },
    },
    ...connector.capabilities === undefined ?
        {} :
        {
          capabilities: requireStringArray(
            ...
            `CMS works connector manifest ${index} capabilities`,
          ),
        },
  });
}
...
function parseConnectorJob(value: unknown, index: number): PortableConnectorJob {
  const job = requireRecord(value, `CMS works connector job ${index}`);
  return validatePortableConnectorJob({
    id: requireModuleId(job.id, `CMS works connector job ${index} id`),
    connectorId: requireModuleId(job.connectorId, `CMS works connector job ${index} connectorId`),
    mode: requireString(job.mode, `CMS works connector job ${index} mode`) as PortableConnectorJob['mode'],
    createdAt: requireString(job.createdAt, `CMS works connector job ${index} createdAt`),
    ...job.conflictPolicy === undefined ?
        {} :
        {
          conflictPolicy: requireString(
            job.conflictPolicy,
            `CMS works connector job ${index} conflictPolicy`,
          ) as PortableConnectorJob['conflictPolicy'],
        },
    ...job.cursorResource === undefined ?
        {} :
        { cursorResource: requireString(job.cursorResource, `CMS works connector job ${index} cursorResource`) },
    ...job.dryRun === undefined ?
        {} :
        { dryRun: requireBoolean(job.dryRun, `CMS works connector job ${index} dryRun`) },
  });
}
...
function validateConnectorWorks(
  ...
  for (const manifest of checkedManifests) {
    if (connectors.has(manifest.id)) {
      throw new Error(`CMS works connector manifest ${manifest.id} must be declared only once.`);
    }
  ...
  const checkedJobs = jobs.map((job): PortableConnectorJob => {
    if (seenJobs.has(job.id)) {
      throw new Error(`CMS works connector job ${job.id} must be declared only once.`);
    }
    seenJobs.add(job.id);
    const manifest = connectors.get(job.connectorId);
    if (!manifest) {
      throw new Error(`CMS works connector job ${job.id} references missing connector ${job.connectorId}.`);
    }
...
type RequiredStandardSolidCmsWorksOptions = Required<
  Pick<
...
function standardSolidPaths(options: StandardSolidCmsWorksOptions): RequiredStandardSolidCmsWorksOptions {
  return {
    baseUrl: ensureTrailingSlash(new URL(options.baseUrl).href),
    typeIndexPath: resolveSolidPath(options.baseUrl, options.typeIndexPath ?? '.well-known/databox-cms/type-index.ttl'),
    manifestContainerPath: ensureTrailingSlash(resolveSolidPath(
      options.baseUrl,
      options.manifestContainerPath ?? '.well-known/databox-cms/modules/',
    )),
    verticalProfileContainerPath: ensureTrailingSlash(resolveSolidPath(
      options.baseUrl,
      options.verticalProfileContainerPath ?? '.well-known/databox-cms/vertical-profiles/',
    )),
    connectorManifestContainerPath: ensureTrailingSlash(resolveSolidPath(
      options.baseUrl,
      options.connectorManifestContainerPath ?? '.well-known/databox-cms/connectors/',
    )),
    connectorJobContainerPath: ensureTrailingSlash(resolveSolidPath(
      options.baseUrl,
      options.connectorJobContainerPath ?? '.well-known/databox-cms/connector-jobs/',
    )),
    moduleStateContainerPath: ensureTrailingSlash(resolveSolidPath(
      options.baseUrl,
      options.moduleStateContainerPath ?? '.databox/cms/modules/',
    )),
  };
}
...
async function serializeTypeIndex(
  ...
  manifests: readonly StandardSolidCmsWorksResource[],
  verticalProfiles: readonly StandardSolidCmsWorksResource[],
  connectorManifests: readonly StandardSolidCmsWorksResource[],
  connectorJobs: readonly StandardSolidCmsWorksResource[],
): Promise<string> {
  const quads: Quad[] = [
  ...
  for (const manifest of manifests) {
    ...
    quads.push(
      ...
      rdfQuad(registration, FOR_CLASS, CMS.terms.Module),
  ...
  for (const profile of verticalProfiles) {
    ...
    quads.push(
      rdfQuad(namedNode(typeIndexPath), REFERENCES, registration),
      rdfQuad(registration, RDF.terms.type, TYPE_REGISTRATION),
      rdfQuad(registration, FOR_CLASS, namedNode(`${CMS.namespace}VerticalProfile`)),
  ...
  for (const connector of connectorManifests) {
    ...
    quads.push(
      rdfQuad(namedNode(typeIndexPath), REFERENCES, registration),
      rdfQuad(registration, RDF.terms.type, TYPE_REGISTRATION),
      rdfQuad(registration, FOR_CLASS, namedNode(`${CMS.namespace}ConnectorManifest`)),
  ...
  for (const job of connectorJobs) {
    ...
    quads.push(
      rdfQuad(namedNode(typeIndexPath), REFERENCES, registration),
      rdfQuad(registration, RDF.terms.type, TYPE_REGISTRATION),
      rdfQuad(registration, FOR_CLASS, namedNode(`${CMS.namespace}ConnectorJob`)),
...
function parseTypeIndexResourcePaths(
  ...
  for (const registration of subjects(quads, RDF.terms.type, TYPE_REGISTRATION)) {
    ...
    if (instances.length !== 1) {
      throw new Error(`CMS Type Index ${label} registrations must have exactly one solid:instance.`);
    }
    if (instances[0].termType !== 'NamedNode') {
      throw new Error(`CMS Type Index ${label} registrations must point to named resources.`);
    }
  ...
  if (requireAny && paths.length === 0) {
    throw new Error(`CMS Type Index must register at least one cms:${label === 'module' ? 'Module' : label} resource.`);
  }
  return paths;
}

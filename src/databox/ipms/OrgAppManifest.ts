 ...
 * The IPMS serves this based on the org's vertical profile and the app's purpose.
 */
 ...
 * component bundle from the IPMS at runtime.
 */
export interface UiModuleDeclaration {
  readonly id: string;
  readonly moduleId: string;
  readonly label: string;
  readonly ipmsRoute: string;
 ...
 * org + device with a scoped licence. The IPMS validates this on
...
/**
 * Container boot configuration — what the IPMS serves to the WASM
 ...
 * in a Solid pod or IPMS config store.
 */
...
/**
 * Build the container boot configuration — the IPMS serves this to
...
export function buildContainerBootConfig(
  ...
  const availableUiModules = profile.uiModules.filter((module) => {
    ...
    if (licence.scope === 'read-only' && module.ipmsRoute.includes('POST')) {
 ...
 * known network ranges. Here we provide the logic for the IPMS
 * to call.
 */

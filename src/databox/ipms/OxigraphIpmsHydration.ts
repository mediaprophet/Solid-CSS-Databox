...
export interface CanonicalIpmsRdfResource {
...
export interface CanonicalIpmsRdfResourceDescriptor {
  /** Solid resource IRI to read from the canonical ResourceStore. */
  readonly path: string;
}

export interface OxigraphIpmsHydrationOperation {
...
export interface OxigraphIpmsHydrationPlan {
...
export interface OxigraphIpmsHydrationExecutor {
  readonly executeUpdate: (update: string, operation: OxigraphIpmsHydrationOperation) => Promise<void>;
}

/**
 * Reads canonical IPMS RDF resources from a Solid ResourceStore and creates a deterministic Oxigraph hydration plan.
 *
 ...
 */
export async function createOxigraphIpmsHydrationPlanFromSolidStore(
  store: ResourceStore,
  resources: readonly CanonicalIpmsRdfResourceDescriptor[],
): Promise<OxigraphIpmsHydrationPlan> {
  return createOxigraphIpmsHydrationPlan(await Promise.all(resources.map(
    async(resource): Promise<CanonicalIpmsRdfResource> => {
 ...
 */
export async function createOxigraphIpmsHydrationPlan(
  resources: readonly CanonicalIpmsRdfResource[],
): Promise<OxigraphIpmsHydrationPlan> {
 ...
 */
export async function replayOxigraphIpmsHydrationPlan(
  plan: OxigraphIpmsHydrationPlan,
...
async function createGraphReplacementOperation(
  resource: CanonicalIpmsRdfResource,
): Promise<OxigraphIpmsHydrationOperation> {
  if (resource.contentType !== TURTLE) {
    throw new Error(`IPMS Oxigraph hydration only supports text/turtle resources, got ${resource.contentType}.`);
  }
  assertAbsoluteIri(resource.path, 'IPMS Oxigraph hydration resource path');
...
function parseCanonicalTriples(resource: CanonicalIpmsRdfResource): Quad[] {
  let quads: Quad[];
  try {
    quads = new Parser({ baseIRI: resource.path }).parse(resource.turtle);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`IPMS Oxigraph hydration could not parse ${resource.path}: ${message}`);
  }
  if (quads.some((quad): boolean => quad.graph.termType !== 'DefaultGraph')) {
    throw new Error(`IPMS Oxigraph hydration resource ${resource.path} must contain Turtle default-graph triples.`);
  }
  return quads

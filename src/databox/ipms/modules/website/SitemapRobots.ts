 ...
 * (see `databox/solid-ipms-plan.md`, §10.7 / §10.8).
 *
...
/** The IPMS control-plane path, disallowed in robots.txt so the operator surface is never indexed. */
...
export const SITEMAP_ROBOTS_MODULE_MANIFEST: SolidModuleManifest = {
  ...
  capabilities: [
    'ipms:sitemap-render',
    'ipms:robots-render',
    'ipms:portable-core-xml',
  ],
  routes: [],
  configShape: `${IPMS.namespace}SitemapRobotsConfigShape`,
};
 ...
 * IPMS control plane (the protected operator surface must never be indexed), and advertises the
 * sitemap.
 */

...
function assertReputationInput(body: unknown): asserts body is ReputationInput {
  if (!isRecord(body) || !Array.isArray(body.reviews)) {
    throw new TypeError('A reputation request needs a reviews array.');
  }

  for (const review of body.reviews as Record<string, unknown>[]) {
...
export function registerReputationRoutes(router: IpmsModuleRouter<(input: HttpHandlerInput) => Promise<void>>): void {

...
export const CASH_REGISTER_MODULE_MANIFEST: SolidModuleManifest = {
  ...
  capabilities: [
    ...
    'pos:offline-queue',
    'pos:audit-receipt',
    'ipms:portable-core-pos-cash-register',
    'ipms:css-enhanced-cash-register-store',
  ],
  routes: [ 'POST /.databox/ipms/pos/register/sessions', 'GET /.databox/ipms/pos/register/sessions' ],
  configShape: `${IPMS.namespace}CashRegisterConfigShape`,
  adminUi: {
...
function sessionSubject(sessionId: string): string {
  return `urn:solid-server:databox:ipms:pos:cash-register-session:${encodeURIComponent(sessionId)}`;
}

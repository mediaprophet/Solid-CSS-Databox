...
export function buildNativeReceiptPrintJob(document: ReceiptDocBase): NativeReceiptPrintJobDescriptor {
  return {
    ...
    status: 'unavailable',
    unavailableReason:
      'No Rust/native-edge printer connector is attached to this IPMS control plane.',
    target: {
 ...
 * to the consumer's digital RDF/VC receipt in the pod (see `databox/solid-ipms-plan.md`, section 10.5).

# Kubernetes IPMS Manifest Skeleton
...
The manifests keep the IPMS profile opt-in by setting `CSS_CONFIG=config/ipms/ipms-file.json` only in the IPMS
Deployment. They mount persistent data at `/data` and read the IPMS control token from a Kubernetes Secret.

...
to implementation. The project now has a broad verified IPMS/domain library plus the first integration layer.
Do not evaluate future work only by whether unit tests pass; evaluate it against the four invariants in §13.1.

Implemented in the current branch:

- Opt-in IPMS/SPARQL presets exist alongside the default/basic CSS install path.
...
- Portable IPMS "works" export/import exists for manifests, enabled/config state, Turtle state, vertical profiles,
...
- `OxigraphIpmsSync` exists as a disabled-by-default helper for syncing allowlisted canonical Solid RDF writes into
  hydrated named graphs.
- `OxigraphIpmsSyncComposition` wires the sync helper into opt-in Components.js lifecycle/config with startup
  hydration, canonical resource allowlists, notification/finalizer ownership, and a SPARQL Update executor.
- `IpmsMigrationProof` demonstrates the in-memory no-lock-in loop: file-backed IPMS works -> standard Solid RDF
  resources -> Oxigraph hydration profile -> vanilla Solid degradation import.
- `scripts/run-ipms-migration-proof.mjs` provides an optional live migration harness for file-backed CSS ->
...
IPMS builds the *pure* pieces — the printable receipt **document** and the **QR payload** (a URL to the
consumer's digital RDF/VC receipt in the pod: `src/databox/ipms/modules/receipt/ReceiptDoc.ts`); the Rust core
...
- **Query:** SPARQL over the hydrated environment for IPMS/admin query power; Comunica-over-LDP is the vanilla
...
**Implementation consequence:** every IPMS module state/config endpoint must write ordinary RDF resources through

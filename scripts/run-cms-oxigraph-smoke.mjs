...
async function runJest(mode, endpoints) {
  ...
  if (mode === 'split') {
    console.log(`Running live CMS smoke in split mode: query=${endpoints.query}, update=${endpoints.update}`);
  } else {
    console.log(`Running live CMS smoke in unified mode: sparql=${endpoints.sparql}`);
  }
...
function skip(reason) {
  console.log(`Skipping live CMS Oxigraph smoke: ${reason}`);
}

...
const DEFAULT_CONTROL_TOKEN = 'cms-migration-control-token-00000001';
...
export async function main(rawArgs, env = process.env) {
  ...
  try {
    ...
    if (plan.sourceCss.launch) {
      ...
      children.push(await startProcess('file-backed CMS', plan.sourceCss.command, plan.sourceCss.args));
    }
    if (plan.targetCss.launch) {
      await mkdir(plan.targetCss.dataPath, { recursive: true });
      children.push(await startProcess('Oxigraph-backed CMS', plan.targetCss.command, plan.targetCss.args));
    }

    await waitForCms(plan.sourceCss.baseUrl, plan.sourceCss.controlToken, plan.timeoutMs);
    await waitForCms(plan.targetCss.baseUrl, plan.targetCss.controlToken, plan.timeoutMs);
...
export function buildMigrationPlan(args, env = process.env) {
  ...
  const targetProfile = mode === 'split' ? 'config/cms/cms-oxigraph.json' : 'config/cms/cms-sparql.json';
  const sourceDataPath = args.sourceDataPath ?? '.data/cms-migration/source';
  const targetDataPath = args.targetDataPath ?? '.data/cms-migration/target';
  let skipReason;

  if (skipWhenUnavailable && !startSourceCss && !args.sourceBaseUrl) {
    skipReason = [
      'No file-backed CMS source endpoint or --start-source-css/--start-css launcher was configured.',
    ...
    skipReason = [
      'No Oxigraph-backed CMS target endpoint or --start-target-css/--start-css launcher was configured.',
  ...
  return {
    ...
    sourceCss: {
      ...
      worksRoute: `${sourceBaseUrl}.databox/cms/works`,
    },
    targetCss: {
      ...
      worksImportRoute: `${targetBaseUrl}.databox/cms/works/import`,
      worksRoute: `${targetBaseUrl}.databox/cms/works`,
      typeIndexRoute: `${targetBaseUrl}.well-known/databox-cms`,
    },
    oxigraph,
    oxigraphEndpoints,
    liveSteps: [
      'seed source CMS module state through the protected CSS control plane',
...
async function runLiveMigration(plan) {
  ...
  const sourceState = await cmsJson(`${plan.sourceCss.baseUrl}.databox/cms/modules/hosting`, {
    method: 'PUT',
    token: plan.sourceCss.controlToken,
    body: {
      enabled: false,
      configTurtle: [
        '<> <urn:example:cmsMigrationHarness> "file-backed-source" .',
        `<> <urn:example:cmsMigrationMarker> "${marker}" .`,
      ].join('\n'),
    },
  });
  const sourceWorks = await cmsJson(plan.sourceCss.worksRoute, {
    method: 'GET',
    token: plan.sourceCss.controlToken,
  });
  const importResult = await cmsJson(plan.targetCss.worksImportRoute, {
    method: 'POST',
    token: plan.targetCss.controlToken,
    body: sourceWorks,
  });
  const targetWorks = await cmsJson(plan.targetCss.worksRoute, {
    method: 'GET',
    token: plan.targetCss.controlToken,
  });
...
function assertMigrationResult(result) {
  ...
  if (result.sourceState.configTurtle?.includes(result.marker) !== true) {
    throw new Error('The source file-backed CMS did not persist the migration marker.');
  }
  ...
  if (!String(result.importResult.type).includes('DataboxCmsWorks')) {
    throw new Error('The target import route did not return a Databox CMS works bundle.');
  }
  ...
  if (!result.hostingManifest.includes('cms:Module')) {
...
async function waitForCms(baseUrl, token, timeoutMs) {
  await waitForHttp(
    `${baseUrl}.databox/cms/modules`,
    timeoutMs,
...
function redactArgs(args) {
  return args.map((arg, index) => args[index - 1] === '--cmsControlToken' ? '<redacted>' : arg);
}

function formatResult(result) {
  return [
    'Live CMS migration proof completed.',
...
function skip(reason) {
  console.log(`Skipping live CMS migration proof: ${reason}`);
}
